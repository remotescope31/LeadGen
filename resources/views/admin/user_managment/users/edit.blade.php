    ...
    <div class="container">
        ...
        <form class="form-horizontal" action="{{ route('admin.user_managment.users.update',$user) }}" method="post">
            ...
            <select class="form-control"  name='role' id="sel1">
                @if( Auth::user()->role >= 0)    <option value="0" @if($user->role == 0) selected @endif>Агент</option> @endif
                @if( Auth::user()->role >= 1)  <option value="1" @if($user->role == 1) selected @endif>Менеджер</option> @endif
                @if( Auth::user()->role >= 2) <option value="2" @if($user->role == 2) selected @endif>Администратор</option> @endif
                @if( Auth::user()->role >= 3) <option value="3" @if($user->role == 3) selected @endif>Super Администратор</option> @endif
            </select>
